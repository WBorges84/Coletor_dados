...
{$IFDEF ANDROID}
,Androidapi.Helpers, Androidapi.JNI.JavaTypes, Androidapi.JNI.Os, System.Types,
  System.SysUtils
{$ENDIF}
...
{$IF CompilerVersion >= 35.0}
    // after Delphi 11 Alexandria
procedure T99Permissions.CameraPermissionRequestResult(Sender: TObject;
  const APermissions: TClassicStringDynArray;
  const AGrantResults: TClassicPermissionStatusDynArray);
{$ELSE}
    // before Delphi 11 Alexandria
...
begin
  if (Length(AGrantResults) = 1) and
    (AGrantResults[0] = TPermissionStatus.Granted) then
  begin
    FrmCamera.CameraComponent.Active := false;
    FrmCamera.CameraComponent.Quality := FMX.Media.TVideoCaptureQuality.MediumQuality;
    FrmCamera.CameraComponent.Kind := FMX.Media.TCameraKind.BackCamera;
    FrmCamera.CameraComponent.FocusMode := FMX.Media.TFocusMode.ContinuousAutoFocus;
    FrmCamera.CameraComponent.Active := True;
    //lblScanStatus.Text := '';
    //Memo1.Lines.Clear;
  end
  else
...
{$IF CompilerVersion >= 35.0}
    // after Delphi 11 Alexandria
procedure T99Permissions.ExplainReason(Sender: TObject;
